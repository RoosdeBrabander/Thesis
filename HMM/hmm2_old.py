# === 2-state HMM on weekly S&P 500 data + regime-specific GBM sims ===
...
# Order states by mean: lower mean = bear, higher = bull
...
summary = pd.DataFrame({
    "State": [f"S{i+1}" for i in range(2)],
    "Mean (weekly)": means_w,
    "Vol (weekly)": stds_w,
    "Mean (annual)": means_a,
    "Vol (annual)": stds_a
...
viterbi_raw = hmm.predict(X)
label_map = {orig: new for new, orig in enumerate(order)}
viterbi = np.vectorize(label_map.get)(viterbi_raw)

# ---------------------------
# 4) Basic return stats
# ---------------------------
mean_w = rets.mean()
std_w  = rets.std()
print("\n=== Basic returns stats ===")
...
plt.scatter(rets.index, prices.loc[rets.index, "price"], c=viterbi, s=10, cmap='coolwarm', label="Regime (Viterbi)")
plt.title("S&P 500 Price with Inferred Regimes (2-state HMM)")
...
# Posterior probability per state
for i in range(2):
...
# ---------------------------
# 6) Regime-specific GBM simulations (no switching)
# ---------------------------
...
plt.figure(figsize=(10,5))
plt.scatter(df["Date"], rets, c=viterbi, s=10, cmap='coolwarm')
plt.axhline(0, color="gray", lw=1)
plt.title("Weekly Log Returns with Regime Coloring (2-state HMM)")
plt.xlabel("Date"); plt.ylabel("Log Return")
plt.tight_layout(); plt.show()
